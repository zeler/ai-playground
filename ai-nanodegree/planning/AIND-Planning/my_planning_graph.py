        ...
        for action in self.all_actions:
            ...
            if node.prenodes.issubset(self.s_levels[level]):
                ...
                for parent_node in self.s_levels[level]:
                    if parent_node in node.prenodes:
        ...
        for node in self.a_levels[level - 1]:
            for literal in node.effnodes:
                ...
                for parent_node in self.a_levels[level - 1]:
                    if literal in parent_node.effnodes:
                        literal.parents.add(parent_node)
                        parent_node.children.add(literal)
